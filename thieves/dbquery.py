...
class Dbquery(object):
    ...
    def gameids_in_range(self, acctID, start=None, end=None):
        """ Get list of gameIDs in database within date range or 1 if not in database. """
        if start is None:
            sqlstart = 0
        else:
        ...
        sql = f"""SELECT GameID FROM matches
             WHERE CAST(SUBSTR(Start,1,8) AS INTEGER) BETWEEN {sqlstart} AND {sqlend}
                AND {acctID} IN (Play1, Play2, Play3, Play4, Play5, Play6)"""
        cur = self.con.cursor()
