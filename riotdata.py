...
class Riotdata(object):
    ...
    def acct_info(self, acct):
        """ Get account info from API and format for database. """
        while True:
            try:
                acctdto = self.watcher.summoner.by_account(self.reg, acct)
            except HTTPError as err:
                if err.response.status_code == 429:
                    print('Too many requests, waiting 15 seconds...')
                    time.sleep(15)
                    ...
                    raise
            break
        return [acct, acctdto['name'], None, self.today]
    ...
    def match_in_range(self, summ, start, end, cached=None):
        ...
        for i in range(1, iters + 1):
            while True:
                try:
                    ...
                    if err.response.status_code == 429:
                        ...
                        print(f'No match data found for segment {i} of {iters}')
                        gameIDs = []
                    elif err.response.status_code == 503:
                        print('Server delay, retrying...')
                        continue
            ...
            gameIDdiff = len(gameIDs)
            if cached is not None:
                gameIDs = [id for id in gameIDs if id not in cached]
                gameIDdiff = gameIDdiff - len(gameIDs)

            print(f"{len(gameIDs)} new games and {gameIDdiff} cached games found for {epochtodate(stime)[0:8]} to {epochtodate(stime+diffs)[0:8]}")
            ...
            if len(gameIDs) > 0:
                for game in gameIDs:
                    ...
                    while True:
                        try:
                            match = self.watcher.match.by_id(self.reg,game)
                        except HTTPError as err:
                            if err.response.status_code == 429:
                                print('Too many requests, waiting 15 seconds...')
                                time.sleep(15)
                    ...
                    try:
                        ...
                        continue
            stime += diffs
        return gamerows, plrows, allplsl
